		...
		<div class="col-sm-6">
				<table class="tbl_input">
					...
					<tr>
						...
						<td>
							<select style="width:100%;" name="id_rph" class="select_rph" required>
								<?php echo $combo_rph; ?>
							</select>
						</td>
					</tr>			
	...
	<div class="row">
		<div class="col-xs-12">
	 				...
	 				<center>
	          		...
	          		<p style="margin-bottom: 0;">LAPORAN PEMOTONGAN SAPI </p>
					...
					<table id="tblExport" class="table table-striped table-bordered table-hover">
						...
						<tbody>
						<?php 
							if($this->uri->segment(2) == "lihat_kartu_stok_sapi") {  
								$no = 1;
								...
								$data_saldo_in = $this->db->query("SELECT COUNT(*) as qtyIn FROM pengiriman_detail INNER JOIN pengiriman ON pengiriman.id_pengiriman = pengiriman_detail.id_pengiriman WHERE pengiriman.tanggal_terima <='$yesterday' OR pengiriman_detail.tanggal_potong <='$yesterday' AND pengiriman.id_rph = '$id_rph'")->row();

								$data_saldo_out = $this->db->query("SELECT COUNT(*) as qtyOut FROM pengiriman_detail INNER JOIN pengiriman ON pengiriman.id_pengiriman = pengiriman_detail.id_pengiriman WHERE pengiriman_detail.tanggal_potong <='$yesterday' AND pengiriman.id_rph = '$id_rph'")->row();
						...
						<?php
								...
								foreach($period as $dt) { 
									...
									$data_masuk = $this->db->query("SELECT COUNT(*) as total_sapi,nota,tanggal_terima FROM pengiriman_detail INNER JOIN pengiriman ON pengiriman.id_pengiriman = pengiriman_detail.id_pengiriman WHERE pengiriman.tanggal_terima ='$date_range' AND pengiriman.id_rph = '$id_rph' AND status_potong = '0' GROUP BY nota");

									$data_keluar = $this->db->query("SELECT COUNT(*) as total_sapi,nota,tanggal_terima,tanggal_potong FROM pengiriman_detail INNER JOIN pengiriman ON pengiriman.id_pengiriman = pengiriman_detail.id_pengiriman WHERE pengiriman_detail.tanggal_potong ='$date_range' AND pengiriman.id_rph = '$id_rph' AND status_potong = '1' GROUP BY nota");

									foreach($data_masuk->result_array() as $data_in) {
										$total_in += $data_in['total_sapi']
								?>

										<tr>
											<td><?php echo $no; ?></td>
											<td><?php 
													$tgl_db_in = explode("-", $data_in['tanggal_terima']); 
													$tgl_convert_in = $tgl_db_in[2]."/".$tgl_db_in[1]."/".$tgl_db_in[0];
													echo $tgl_convert_in;
												?>
											</td>
											...
											<td>Sapi Masuk</td>
											<td><?php echo $data_in['total_sapi']; ?></td>
											<td>-</td>
											<td>-</td>
											<td>-</td>
											<td><?php $saldo_awal = $saldo_awal + $data_in['total_sapi']; echo $saldo_awal; ?></td>
										</tr>
					<?php 	$no++;	}

									foreach($data_keluar->result_array() as $data_out) {
										$total_out += $data_out['total_sapi']
								?>
										...
										<tr>
											...
											<td><?php echo $data_out['total_sapi']; ?></td>
											<td><?php $saldo_awal = $saldo_awal - $data_out['total_sapi']; echo $saldo_awal; ?></td>
										</tr>
					<?php 			}
						 		}
							} ?>
