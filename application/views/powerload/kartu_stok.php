<div class="col-md-12">	
	...
	<div class="row">
		<div class="col-xs-12">
	 				...
	 				<center>
	          			<a style="margin-bottom:20px;border-radius:25px;" class="btn btn-primary btn-sm" onclick="exportPowerload('xls');" href="javascript://"><span class="glyphicon glyphicon-export"> </span> Export to Excell</a>
	          		</center>
					...
					<table class="table table-striped table-bordered table-hover">
						...
						<?php 
							if($this->uri->segment(2) == "lihat_kartu_stok") {  
								...
								$data_saldo_in = $this->db->query("SELECT tanggal,SUM(merah) as sd_merah,SUM(orange) as sd_orange,SUM(hitam) as sd_hitam,SUM(kuning) 
																as sd_kuning FROM powerload  WHERE tanggal <='$yesterday' AND id_rph = '$id_rph'");
								...
								$data_saldo_out = $this->db->query("SELECT tanggal_potong,SUM(merah) as sd_merah,SUM(orange) as sd_orange,SUM(hitam) 
																as sd_hitam,SUM(kuning) as sd_kuning FROM penerimaan_detail INNER JOIN pengiriman 
																ON penerimaan_detail.id_pengiriman = pengiriman.id_pengiriman WHERE penerimaan_detail.tanggal_potong 
																<='$yesterday' AND pengiriman.id_rph = '$id_rph' AND status_potong >= '1' GROUP BY tanggal_potong");
					...
					<?php
						...
						$data_single_in = $this->db->query("SELECT tanggal,SUM(merah) as sd_merah,SUM(orange) as sd_orange,SUM(hitam) as sd_hitam,SUM(kuning) as sd_kuning 
															FROM powerload  WHERE tanggal <='$tanggal2' AND id_rph = '$id_rph'"); 
